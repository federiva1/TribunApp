...
- **Static fixture data** (club.html VER FECHAS section): `data/fixtures/{slug}.json` — array of `{ num, fecha, rival, local, goles }` where `goles` is `[argGoals, rivalGoals]` or `null` if unplayed. Must be created manually per club.
...
### VER FECHAS access control
- **Guests**: `renderCurrentFechaGuest()` — shows only the current fecha (based on `CLUB.rivalActual`), fetches Supabase formations/scores for that rival, plus a CTA to register.
- **Logged-in**: `renderFechas()` — loads `data/fixtures/{slug}.json`, renders all played matches as a clickable list. Clicking a past match opens `openPrevFecha()` which fetches Supabase data for that rival.
...
- `data/fixtures/{slug}.json` must be created manually for each club before VER FECHAS works for logged-in users. Only `argentinosjuniors` has this file currently.
