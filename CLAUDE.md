...
5. **S/P (liga)** — los suplentes pueden recibir "sin puntaje" (`null` en el JSON de `puntajes`, misma convención que el Mundial). No cuenta ni para el promedio ni como voto ni como fecha puntuada; todos los cálculos descartan `null`/`''` explícitamente (ojo: `Number(null) === 0`, no NaN). La regla del Mundial en `equipo.html` es distinta (S/P vale 0 o 2 según el %) y quedó pendiente de revisión.
